...
class A5_2(object):
    ...
    def _generate_key_stream_with_predefined_register(self,r1,r2,r3,r4,generated_only_send_key=False):
        ...
        return(self.send_key,self.recive_key)

    def _create_register_backup(self):
        self.initial_state={ 'r1' : copy.deepcopy(self.r1),
    ...
    def _clocking_with_majority(self,limit,generate_key_stream=False,save_register_state=False):
        ...
        for i in range(limit):
            ...
            if generate_key_stream:
                if save_register_state:
                    self.register_stste.append({'r1': copy.deepcopy(self.r1),
                                                'r2': copy.deepcopy(self.r2),
                                                'r3': copy.deepcopy(self.r3),
                                                'r4': copy.deepcopy(self.r4)})
                self._add_key_stream(i)


    def _generate_key_stream(self,save_register_state=False,generate_only_send_key=False):
        
        self._clocking_with_majority(KEY_STREAM_SIZE,save_register_state=save_register_state,generate_key_stream=True)
        if not generate_only_send_key:
            self.send_key=self.key_stream.deep_copy()
            self._clocking_with_majority(KEY_STREAM_SIZE,save_register_state=save_register_state,generate_key_stream=True)
            self.recive_key=self.key_stream.deep_copy()
        else:
            self.send_key=self.key_stream
            self.recive_key=None

    def get_key_stream(self, save_register_state=False, generate_only_send_key=False):
        
        #kc=64 and fn=22
        self._clocking(KEY_SIZE,self.key_stream)
        self._clocking(FRAME_COUNTER_SIZE,self.frame_counter)

        #set forced bit and warming phase
        self._set_bits()
        self._clocking_with_majority(MAJORITY_CYCLES_A52)
        self._create_register_backup()

        #generate key for uplink and downlink
        self._generate_key_stream(save_register_state,generate_only_send_key=generate_only_send_key)
        
        return(self.send_key,self.recive_key)

    def _add_key_stream(self,index):
        
        #out put key stream for index
    ...
    def _majority(self):
        ...
        clocked_bits=[]
        clocked_bits= clocked_bits + self.r4.get_clock_bit()
